...
namespace ProjeOdev2
{
    public partial class SinavSayfa : Form
    {
        ...
        {   
            if(SoruSayisi.Text == toplamSoruSayisi.Text)// aktif soru toplam soru sayisina eşitse sınavı bitir
            {
                sonrakiBtn.Text = "Bitir";
                bitir();
                this.Close();
            }
            else // değilse bir puanı hesapla ve bir sonraki soruya geç
            {
                PuanHesapla();
        ...
        private void PuanHesapla()
        {
            ...
            double  p = sınav1.sorular[Sindis].puan; // soru puanı
            string c = sınav1.sorular[Sindis].cevap;// sorunun cevabı
